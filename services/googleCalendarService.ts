...
app.get('/auth/google/callback', async (req, res) => {
  ...
  try {
    ...
    console.log('Google 인증 성공! 토큰:', tokens);
...
// 할 일 목록 가져오기 (이 예제에서는 간단함을 위해 구현하지 않습니다.)
app.get('/api/todos', async (req, res) => {
    // 실제 앱에서는 여기서 Google Calendar API를 호출하여
    // 특정 기간의 이벤트를 가져와 할 일 목록으로 변환해야 합니다.
    // 예: calendar.events.list(...)
    res.json([]);
});
  ...
  try {
    const event = {
      ...
      start: {
        ...
        dateTime: new Date(new Date(dateTime).getTime() + 60 * 60 * 1000).toISOString(), // 1시간짜리 이벤트로 설정
        timeZone: 'Asia/Seoul',
      },
      reminders: {
        useDefault: false,
    ...
    console.log('캘린더 이벤트 생성됨:', response.data);
